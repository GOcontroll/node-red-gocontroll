module.exports = function (RED) {
	...
	function GOcontrollControllerSupply(config) {
		...
		if (adc == 0) {
			...
			var devices_dir = fs.opendirSync("/sys/bus/iio/devices/");
			var device_dir = null;
			while (device_dir = devices_dir.readSync()) {
				try {
					if (fs.readFileSync("/sys/bus/iio/devices/" + device_dir.name + "/name", 'utf-8').trim() === "mcp3004") {
						adcpath = "/sys/bus/iio/devices/" + device_dir.name;
						break
					}
				} catch { }
			}
		...
		function getVoltages() {
			if (adc == 1) {
				...
				try {
					msgOut["K15A"] = parseInt((((parseInt(fs.readFileSync(adcpath + "/in_voltage0_raw"))) * decimalFactor) / 1.5) * 11700); //KL15-A
				} catch (err) {
				}

				try {
					msgOut["K15B"] = parseInt((((parseInt(fs.readFileSync(adcpath + "/in_voltage1_raw"))) * decimalFactor) / 1.5) * 11700); //KL15-B
				} catch (err) {
				}

				try {
					msgOut["K15C"] = parseInt((((parseInt(fs.readFileSync(adcpath + "/in_voltage2_raw"))) * decimalFactor) / 1.5) * 11700); //KL15-C
				} catch (err) {
				}

				try {
					msgOut["batteryVoltage"] = parseInt((((parseInt(fs.readFileSync(adcpath + "/in_voltage3_raw"))) * decimalFactor) / 1.5) * 11700); //Supply
				} catch (err) {
				}
