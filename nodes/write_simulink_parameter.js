    ...
    function GOcontrollWriteSimulink(config) {
        ...
        function getHeader() {
            try {
                ...
                var headerFile = fs.readFileSync("/usr/node-red-static/header.json");
        ...
        function getParams() {
            try {
                //check if the address or size of the signal has changed due to a recompilation of the model
                var parameterFile = fs.readFileSync("/usr/node-red-static/parameters.json");
            } catch (err) {
                if (parseA2L()) {
                    return getParams();
                } else {
        ...
        function parseA2L() {
            var parseResult = shell.exec("go-parse-a2l");
            if (!parseResult.stdout.includes("succesfully")) {
                parseResult = shell.exec("python3 /usr/moduline/python/parse_a2l.py");
                if (!parseResult.stdout.includes("succesfully")) {
                    return false;
                }
            }
